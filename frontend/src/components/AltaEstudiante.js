...
let AltaEstudiante = () => {
    ...
    let interpretaExcel = async (e) => {
        try {
            let resp = await creaAlerta({
                titulo: "Advertencia",
                mensaje: "¿Está seguro de cargar el excel seleccionado?",
                icono: 5,
                boolBtnCancel: true,
    ...
    let ejemplo = async () => console.log("Descargar ejemplo");
    ...
    let peticion = async (bool) => {
        try {
            ...
            const resultado = await ObjAjax.altaEstudiantes(datosFormulario, setEspera);
